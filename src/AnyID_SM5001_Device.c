...
const u8 DEVICE_VERSION[DEVICE_VERSION_SIZE]@0x08005000 = "SM5001 23082301 GD322302";
...
void Device_ReadDeviceParamenter(void)                             		     //OK
{
    ...
    if(b)
    {
        ...
        //¼ì²â²ÎÊýÊÇ·ñÕýÈ·£¬Èç¹û²»ÕýÈ·£¬Ê¹ÓÃÄ¬ÈÏ²ÎÊý²Ù×÷
        if(crc1 != crc2)
        {
            b = FALSE;
        }
	...
	{
		memcpy(g_sFramBootParamenter.currentVerSion, "SM5001_00000000_FFFF", FRAM_VERSION_SIZE);//¶ÁÈ¡²ÎÊýÊ§°Ü£¬»Ö¸´Ä¬ÈÏ
...
u16 Device_ResponseFrame(u8 *pParam, u8 len, READER_RSPFRAME *pOpResult)
{
    ...
    crc = a_GetCrc(pOpResult->buffer, pos); //´ÓLEN¿ªÊ¼¼ÆËãcrc
...
u16 Device_ResponseRtBat(u8 *pBuffer)
{
    ...
    pBuffer[pos++] = UART_FRAME_PARAM_RFU;
    crc = a_GetCrc(pBuffer, pos); //´ÓLEN¿ªÊ¼¼ÆËãcrc
...
u16 Device_ResponseGateFrame(u8 add, u8 mode, READER_RSPFRAME *pOpResult)
{
    ...
    else
    {
        ...
        {
         	if(g_aGateSlvInfo[index].state == GATE_STAT_OK || g_aGateSlvInfo[index].flag == GATE_STAT_OK)
          	{
...
void Device_GateRsvSlvInfo(u8 *pParams, u16 paramsLen, GATE_OPINFO *pOpInfo)
{
    ...
    num = pParams[pos++];
    for(i = 0; i < num; i++)													//num = 1å¯èƒ½é€ æˆï¼Ÿï¼Ÿï¼Ÿï¼Ÿï¼Ÿï¼Ÿ
    {
        pInfo = (GATE_SENSORINFO *)(pParams + pos);									
        infoIdx = (pOpInfo->slvIndex << 1) + pInfo->addr;						//ä¸¤æ¡è¿žç»­åœ°å€ç›¸åŒå¯¼è‡´ä¸€ä»“æ•°æ®ä¸¢å¤±ï¼Ÿï¼Ÿï¼Ÿ
        ...
        else 
        {
            if((memcmp(&pInfo->sensorState, &pPreInfo->sensorState, 1) != 0) ||
               ...
               (pInfo->chagInfo.state != pPreInfo->chagInfo.state))     //×´Ì¬·¢Éú±ä»¯
            {
                g_aGateSlvInfo[infoIdx].bTxInfo = TRUE;//
            }
            else
		...
		if(g_aGateSlvInfo[infoIdx].flag != GATE_STAT_OK)				//æŽ’é™¤å‚æ•°å¹²æ‰°ï¼Œ
		{
			g_aGateSlvInfo[infoIdx].flag = GATE_STAT_OK;
		}
...
BOOL Device_GateProceRspFrame(u8 *pFrame, GATE_OPINFO *pOpInfo, u32 tick)
{
    ...
    if((addr == pOpInfo->slvIndex + 1 && pFrame[UART_RFRAME_POS_CMD] == pOpInfo->cmd) || (batAddr == addr && pFrame[UART_RFRAME_POS_CMD] == GATE_FRAME_CMD_RTNBAT)
       || (batAddr == addr && pFrame[UART_RFRAME_POS_CMD] == GATE_FRAME_CMD_BRWBAT))    //½è»¹µç³ØÖ¸ÁîÊÇ·ñÇø·Ö£¿
    {
        b = TRUE;
        ...
        if(pFrame[UART_RFRAME_POS_CMD] == GATE_FRAME_CMD_RTNBAT)                        //»»µç¹ý³ÌÔÝÊ±´¦Àí
        {
        ...
        switch(pOpInfo->cmd)
        {
            ...
            case GATE_FRAME_CMD_CHARGE:
                ...
                a_ClearStateBit(g_sDeviceRspFrame.mark, DEVIDE_MARK_GATE);
                pOpInfo->slvCmd.cmd = 0;            //²Ù×÷Íê³É£¬Çå¿Õ²Ù×÷
                Device_AtRsp(W232_CNT_TIME_500MS, W232_CNT_REPAT_NULL, W232_MQTT_TOPIC_CMD);
            break;
            ...
            if(paramsLen == 2)
            {
                    ...
                    else
                    {
						...
						if(pParams[0] == 0x00)
						{
							...
							Device_AtRsp(W232_CNT_TIME_500MS, W232_CNT_REPAT_NULL, W232_MQTT_TOPIC_BWBAT);
							Device_AtRsp(W232_CNT_TIME_500MS, W232_CNT_REPAT_NULL, W232_MQTT_TOPIC_BWBAT);//------·ÀÖ¹Êý¾Ý·¢ËÍÖÐÍ¾¶ªÊ§
            ...
            case GATE_FRAME_CMD_SET_PARAMS:   //  10
              if(paramsLen > 0)
              {
                    //°æ±¾ÐÅÏ¢£¬Ò»¿é²Ö¿Ø¿ØÖÆÁ½¸ö²Ö£¬°æ±¾ÐÅÏ¢Ò»ÖÂ
          ...
          case GATE_FRAME_CMD_PLANE_BAT:
                if(paramsLen == DEVICE_FRAME_LEN_RETURN_BAT + 1)
                {
                    if(Device_ChkGate(add))
                    {
						 ...
						 {
						 	g_sDeviceRspFrame.err = READER_RESPONSE_ERR_DEVICE; //²Ù×÷Ã¦
						 }
                    }
                    else
                    {                             		                             		            
                ...
                break;
          case GATE_FRAME_CMD_GET_ININFO:
                if(paramsLen == DEVICE_FRAME_LEN_ACTCTL_CTL)                                 //ÐÞ¸Ä£¿£¿£¿£¿£¿£¿
                {
					{
...
void Device_ServerProcessRxInfo(W232_RCVBUFFER *pRcvBuffer, u32 tick)               //´¦Àí·þÎñÆ÷ÏÂ·¢Êý¾Ý
{
    ...
    { 
		...
		else if(pRcvBuffer->len == 0)    //³¤¶ÈÎª0£¬²»Ð£Ñé£¿£¿£¿
		{
		   crc1 = crc2 = 0xFF;
		}
		else
...
void Device_UpHeartData(u32 id, u8 *pBuffer, char *strAtBuff, char *strRspBuff, u16 lenth)
{
    ...
    sprintf(strRspBuff,"{\"id\":%.8d,\"dp\":{\"device\":[{\"v\":\"FF20%.4d%s\"}]}}",id, lenth, strRspbuffer);//bug£¬³¤¶ÈÓ¦¸ÃÊÇÊ®Áù½øÖÆ
...
u16 Device_HeartFormat(u8 *pBuffer, u32 tick)
{
  ...
  //¹ñÌåÐÅÏ¢      
    ...
    memcpy(pBuffer + pos, (u8 *)&DEVICE_VERSION[DEVICE_VERSION_LEN - 1], DEVICE_VERSION_LEN);  //Ó²¼þ°æ±¾
	pos += DEVICE_VERSION_LEN ;
    memcpy(pBuffer + pos, (u8 *)&DEVICE_VERSION, DEVICE_VERSION_LEN - 2);						//Èí¼þ°æ±¾
	pos += (DEVICE_VERSION_LEN - 2);
	pBuffer[pos++] = 0x30;
	pBuffer[pos++] = 0x30;					//  Èí¼þ°æ±¾ÐÅÏ¢²¹0
  ...
  //²ÖÌåÐÅÏ¢
    ...
    pBuffer[pos++] = g_sW232Connect.sigNum;//ÐÅºÅÇ¿¶È
...
BOOL Device_CheckRsp(W232_CONNECT *pCntOp, u8 *pRxBuf, u8 len)
{
    ...
    if(strstr((char const *)pRxBuf, g_aStrBuffRequst) != NULL)
    {
        ...
        if((*(pCntOp->requestId +  DEVICE_MQTT_FRAME_CMDID_TAG_1) == DEVICE_MQTT_FRAME_CMDID_MASK) &&        //¹ýÂËËð»µÊý¾ÝÖ¡£¬·ñÔò±»·þÎñÆ÷Ìß³ö
           ...
           (*(pCntOp->requestId +  DEVICE_MQTT_FRAME_CMDID_TAG_4) == DEVICE_MQTT_FRAME_CMDID_MASK))
        {
              ...
              memcpy(pCntOp->requestBuffer,  (u8 *)(idStrAdd + W232_RQUEST_ID_LEN + W232_RQUEST_ID_DTU_LEN), W232_RQUEST_BUFFER_LEN);   //×ö³¤¶ÈÐ£Ñé
             if(W232_DataHandle(&g_sW232RcvBuffer, (u8 *)(idStrAdd + W232_RQUEST_ID_LEN + W232_RQUEST_ID_DTU_LEN)))
             {
				 g_sDeviceParams.offLineTime = 0;				//åŒæœåŠ¡å™¨æœ‰æ•°æ®äº¤äº’ï¼Œåˆ¤å®šæ­£å¸¸
                 bOK = TRUE;
             }
        }
		else
		{
			memset(pRxBuf, 0, (u8 *)idStrAdd - pRxBuf);													//¹ýÂËµÚÒ»²ãËð»µÊý¾Ý£¬¼ÌÐø²éÑ¯
			...
			{
				bOK = FALSE;																							//Á½´Î²éÑ¯£¬25ms³¬Ê±£¬»¹³öÏÖÊý¾ÝËð»µ£¬ÊÓÎª´íÎóÊý¾Ý£¬·ÅÆú
			}
		}
    }
...
void Device_GateStateRsp()    //×´Ì¬±ä»¯·´À¡£¬ÐÄÌø¸üÐÂ£¬ÊÇ·ñÑ¹Á¦¹ý´ó
{ 
	u8 index = 0;
	if(index < (g_sDeviceParams.gateNum << 1))
	{
	    for(index = 0; index < (g_sDeviceParams.gateNum << 1); index ++)
	    {
			if(memcmp(&(g_aGateSlvStat[index].sensorState), &(g_aGateSlvInfo[index].sensorInfo.sensorState), sizeof(GATE_STATINFO)) ||
				...
				memcmp(&(g_aGateSlvStat[index].chagState), &(g_aGateSlvInfo[index].sensorInfo.chagInfo.state), sizeof(u8)))     //×´Ì¬±ä»¯
			{
...
u8 Device_Ota_Token(char *ver, char *res, unsigned int et, char *access_key, char *authorization_buf, unsigned short authorization_buf_len)
{
	...
	char sign_buf[40];							//±£´æÇ©ÃûµÄBase64±àÂë½á¹û ºÍ URL±àÂë½á¹û
	char hmac_sha1_buf[40];						//±£´æÇ©Ãû
	char access_key_base64[40];					//±£´æaccess_keyµÄBase64±àÂë½áºÏ
	char string_for_signature[56];				//±£´æstring_for_signature£¬Õâ¸öÊÇ¼ÓÃÜµÄkey
...
//----------------------------------------------------½«¼ÓÃÜ½á¹û½øÐÐBase64±àÂë------------------------------------------------------
	olen = 0;
	memset(sign_buf, 0, sizeof(sign_buf));
	BASE64_Encode((unsigned char *)sign_buf, sizeof(sign_buf), &olen, (unsigned char *)hmac_sha1_buf, strlen(hmac_sha1_buf));

//----------------------------------------------------½«Base64±àÂë½á¹û½øÐÐURL±àÂë---------------------------------------------------
...
void Device_FormatMainInfo(GATE_OPINFO *pGateOpInfo)
{
    ...
    pBuffer[pos++] = (g_sIoInfo.sersorState & 0x1F) | 
                     ...
                     (Device_ChkDeviceStat(IO_DEVICE_STAT_LED) << 6) | 
                     (Device_ChkDeviceStat(IO_DEVICE_STAT_RELAY) << 7);
    pBuffer[2] = pos - 3 + 2; //¼õÈ¥Ö¡Í·7E 55 LEN µÄÈý¸ö×Ö½Ú£¬¼ÓÉÏCRCµÄÁ½¸ö×Ö½Ú

    crc = a_GetCrc(pBuffer + UART_FRAME_POS_LEN, pos - 2); //´ÓLEN¿ªÊ¼¼ÆËãcrc
