...
void CustomHID_Reset(void)
{
  ...
  /* Initialize Endpoint 1 */
     SetEPType(ENDP1, EP_INTERRUPT);//端点1 输出 PC向stm32发送数据 
     SetEPRxAddr(ENDP1, ENDP1_RXADDR);
     SetEPRxCount(ENDP1, USB_FRAME_LEN);
     SetEPRxStatus(ENDP1, EP_RX_VALID);

  /* Initialize Endpoint 2 */
     SetEPType(ENDP2, EP_INTERRUPT);//端点2 输入 Stm32向pc发送数据
     SetEPTxAddr(ENDP2, ENDP2_TXADDR);
     SetEPTxCount(ENDP2, USB_FRAME_LEN);
